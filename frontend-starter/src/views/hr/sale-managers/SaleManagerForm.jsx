...
const SaleManagerForm = () => {
  ...
  useEffect(() => {const getSupplier = async () => {
    setLoading(true)
    await axiosClient.get(`/sales-managers/${id}`).then(res => {
      setLoading(false)
  ...
  const handleSubmit = (e) => {
    ...
    const store = async () => {
      setLoading(true)
      await axiosClient.post('/sales-managers', input).then(res => {
        setLoading(false)
    ...
    const update = async () => {
      setLoading(true)
      await axiosClient.put(`/sales-managers/${id}`, input).then(res => {
        setLoading(false)
        toastAlert(res.data.message)
        navigate('/sales-managers')
      }).catch(err => {
    ...
    return (
        <DefaultLayout title={id ? `Edit Sales Manager` : 'Add Sales Manager'}>
          <section className="content">
            <div className="container-fluid">
              <div className="row">
                <div className="col-12">
                  <div className="card">
                    <CardHeader isForm title={id ? 'Edit Sales Manager' : 'Add Sales Manager'} link='/sales-managers' />
                    {/* /.card-header */}
